        ...
        <PopoverTrigger asChild>
          ...
          >
            {isLoading ? (
              ...
              state.value || placeholder || 'Pilih atau buat baru...'
            )}
            <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
          </Button>
        </PopoverTrigger>
        <PopoverContent className="p-0 min-w-[--radix-popover-trigger-width] max-h-[calc(var(--radix-popover-content-available-height)-1rem)] overflow-auto">
          <Command>
            <CommandInput
              value={state.query}
              onValueChange={(query) => dispatch({ type: 'SET_QUERY', payload: query })}
              placeholder={placeholder || 'Cari atau buat baru...'}
              className="h-9"
            />
            <CommandGroup>
              {state.query &&
