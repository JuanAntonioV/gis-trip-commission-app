...
const EditLocationForm = () => {
    ...
    const initialData = serverProps.location as Location;
    ...
    const { data, setData, put, errors, processing, recentlySuccessful, resetAndClearErrors } = useForm<Required<FormType>>({
        ...
        location_type_id: initialData.location_type_id || -1,
    });

    const submit: FormEventHandler = (e) => {
        e.preventDefault();

        put(route('locations.update', initialData.id), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const [location, setLocation] = useState<Poi | null>(
        !!initialData.latitude && !!initialData.longitude
            ? {
                  key: `${initialData.latitude},${initialData.longitude}`,
                  location: { lat: Number(initialData.latitude), lng: Number(initialData.longitude) },
              }
            : null,
    );
    ...
    useEffect(() => {
        ...
        const defaultLocation =
            initialData.latitude && initialData.longitude
                ...
                : { lat: 3.595226750097991, lng: 98.67200113297093 };
